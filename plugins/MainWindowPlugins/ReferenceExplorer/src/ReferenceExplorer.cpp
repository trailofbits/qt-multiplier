...
ReferenceExplorer::ReferenceExplorer(
    ...
    std::shared_ptr<ITreeGenerator> generator, bool show_code_preview,
...
void ReferenceExplorer::InitializeWidgets(
  ...
  if (macro_explorer) {
    ...
    code_model = ICodeModel::Create(file_location_cache, index, true);
  }

  d->code_model = code_model;

  if (highlighter) {
    auto model_proxy = highlighter->CreateModelProxy(
        code_model, ICodeModel::RealRelatedEntityIdRole);

    d->code_view = ICodeView::Create(model_proxy, this);

  } else {
    d->code_view = ICodeView::Create(code_model, this);
  }
