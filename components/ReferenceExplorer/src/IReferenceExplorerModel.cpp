...
void IReferenceExplorerModel::Node::AssignUniqueId(void) {
  node_id = gNextNodeId.fetchAndAddOrdered(1);
}

// Reset the tree and prevent the practical re-use of node IDs.
IReferenceExplorerModel::NodeTree::NodeTree(void)
...
IReferenceExplorerModel::NodeTree::CurrentRootNode(void) {
